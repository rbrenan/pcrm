import { createUser, createSuffix } from "../../lib/prisma";
...
export default withApiAuthRequired(async function handler(req, res) {
  ...
  if (!body.suffix) {
    ...
    return res.status(400).json({ data: "Suffix is required" });
  }

  // Insert new person into the database and redirect to people page
  const newSuffix = await createSuffix(req.body);
  console.log(JSON.stringify(newSuffix));
